    ...
    const handleRemoveVoter = (voterEmail) => {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                axios.patch(`https://electra-poll-server.vercel.app/voters/${data?._id}`, { voterEmail })
                    .then(data => {
                        if (data.status === 200) {
                            Swal.fire({
                                icon: 'success',
            ...
            <dialog id="my_modal_5" className="modal modal-bottom sm:modal-middle">
                ...
                <div className="modal-box">
                    ...
                    <form method="dialog" >
                        ...
                        <button id='modal-close-btn' className="btn btn-sm btn-circle btn-ghost absolute right-2 top-2">✕</button>

                    </form>
                    <form onSubmit={handleSubmit}>
                        <div>
                            <label htmlFor="">Voter Name</label>
                            <input type="text" name='voterName' className='voter-input mt-2' placeholder='Saiful Islam' required />
                        </div>
                        <div className='mt-4'>
                            <label htmlFor="">Voter Email</label>
                            <input type="email" name='email' className='voter-input mt-2' placeholder='example@gmail.com' required />
                        </div>
