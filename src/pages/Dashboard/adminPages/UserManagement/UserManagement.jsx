...
const UserManagement = () => {
  ...
  const { data: data = [], refetch, isLoading } = useQuery(['users', user], async () => {
    const res = await axios.get(`https://electra-poll-server.vercel.app/all-users`)
    return res.data
  })
  const users = data
  ...
  const handleRemoveVoter = (id) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        axios.delete(`https://electra-poll-server.vercel.app/all-users/${id}`)
          .then(data => {
            if (data.status === 200) {
              Swal.fire({
                icon: 'success',
